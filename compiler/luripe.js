      ...
      function luraphBundle(OP, bytecode, checksum, constPool, spilledFns, usedOps) {
        ...
        if (spilledFns.length) {
          ...
          for (const f of spilledFns) {
            ...
            const chunk = "local _s=" + DEC + "(\"" + numStr + "\"," + seed + "," + R + ");" +
              "local _f=" + LSV + "(_s);" +
              "if setfenv and _f then pcall(setfenv,_f," + GV + ") end;" +
              GV + "[" + JSON.stringify(f.sym) + "]=_f and _f();";
            spillPrelude += chunk;
          }
        }
        return (
...
function protect(source, opts) {
  ...
  if (mode === "vm") {
    ...
    return { mode: "vm", output: compile(source) };
  }
  ...
  // Pure-logic candidate: try VM, but if compilation throws, fall back to wrapper.
  try {
    const vmOut = compile(source);
