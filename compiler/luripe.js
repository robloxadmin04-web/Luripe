...
function rid(used) {
  let n;
  do {
    n = "_" + Math.random().toString(36).slice(2, 7);
  } while (used.has(n));
  used.add(n);
  return n;
}
...
function generateVM(OP, usedNames) {
  ...
  const N = {
    decodeString: rid(used),
    decodeNumber: rid(used),
    checksumOf: rid(used),
    BUILTINS: rid(used),
    stack: rid(used),
    sp: rid(used),
  ...
  const body = {
    ...
    ADD:
      Math.random() < 0.5
        ? "local x=" + O + "();local b=" + O + "();" + P + "(b+x)"
        : "local b=" + O + "();local x=" + O + "();" + P + "(x+b)",
    ...
    EQ:
      Math.random() < 0.5
        ? "local x=" + O + "();local b=" + O + "();" + P + "(b==x and 1 or 0)"
        : "local b=" + O + "();local x=" + O + "();" + P + "(x==b and 1 or 0)",
    NE:
      Math.random() < 0.5
        ? "local x=" + O + "();local b=" + O + "();" + P + "(b~=x and 1 or 0)"
        : "local b=" + O + "();local x=" + O + "();" + P + "(x~=b and 1 or 0)",
    NOT:
      "local x=" + O + "();" + P + "((x==0 or x==false or x==nil) and 1 or 0)",
    TLEN: "local t=" + O + "();" + P + "(#t)",
    NEWTABLE: P + "({})",
    SETTABLE:
      "local v=" + O + "();local k=" + O + "();local t=" + O + "();t[k]=v",
    ...
    BUILTIN:
      "local id,ac=a[1],a[2];local args={};for k=ac,1,-1 do args[k]=" +
      O +
      "() end;" +
      P +
      "(" +
      ...
      "[id](args))",
    CALL:
      "local fa,ac=a[1],a[2];local nf={};for k=ac-1,0,-1 do nf[k]=" +
      O +
      "() end;" +
      N.csTop +
      "=" +
    ...
    RETURN:
      ...
      "=true",
    RETURNN:
      "local n=a;local vs={};for k=n,1,-1 do vs[k]=" +
      O +
      "() end;local cl=" +
      N.callStack +
      "[" +
    ...
    STOREMULTI:
      "local sl=a;local n=" +
      O +
      "();local vs={};for k=n,1,-1 do vs[k]=" +
      O +
      "() end;for k=1,#sl do " +
      F +
      "[sl[k]]=vs[k] end",
    VARARG:
      "local st=a;local t={};local n=0;local k=st;while " +
      F +
      "[k]~=nil do n=n+1;t[n]=" +
      F +
      "[k];k=k+1 end;" +
    ...
    NEWCELL: F + "[a]={v=" + O + "()}",
    LOADCELL:
      "local cl=" + F + "[a];" + P + "(type(cl)=='table' and cl.v or cl)",
    STORECELL:
      "local val=" +
      O +
      "();local cl=" +
      F +
      "[a];if cl==nil then cl={v=nil};" +
      F +
      "[a]=cl end;cl.v=val",
    LOADUP:
      "local u=" +
    ...
    STOREUP:
      ...
      " and " +
      N.upvals +
      "[a];if type(u)=='table' then u.v=val end",
    ...
    CLOSURE:
      "local addr=a[1];local nc=a[2];local ups={};for i=1,nc do local c=a[2+i];if c>=0 then ups[i-1]=" +
      F +
      "[c] else ups[i-1]=(" +
      N.upvals +
      " and " +
      ...
      "[-c-1]) end end;" +
      P +
      "({__cl=true,addr=addr,ups=ups})",
    CALLC:
      "local ac=a;local cf={};for k=ac-1,0,-1 do cf[k]=" +
      O +
      "() end;local clo=" +
      O +
      "();" +
    ...
    BRIDGESET:
      ...
      "(a);local val=" +
      O +
      "();local G=(getgenv and getgenv()) or (getfenv and getfenv()) or _G;G.__LRP_BRIDGE=G.__LRP_BRIDGE or {};G.__LRP_BRIDGE[nm]=val",
    SELFCALL:
      "local nm=" +
      N.decodeString +
      "(a[1]);local ac=a[2];local args={};for k=ac,1,-1 do args[k]=" +
      O +
      "() end;local obj=" +
      O +
      "();local fn=obj[nm];" +
      ...
      "else " +
      P +
      "(fn(obj,(table.unpack or unpack)(args,1,ac))) end",
    CALLR:
      "local ac=a;local args={};for k=ac,1,-1 do args[k]=" +
      O +
      "() end;local fn=" +
      O +
      "();" +
      ...
      "else " +
      P +
      "(fn((table.unpack or unpack)(args,1,ac))) end",
    RCALL:
      "local ac=a;local args={};for k=ac,1,-1 do args[k]=" +
      O +
      "() end;local fn=" +
  ...
  if (missing.length)
    throw new Error("missing handler bodies: " + missing.join(","));
  ...
  function mutate(code) {
    ...
    const nums3 = [];
    while (nums3.length < 3) {
      const n = ((Math.random() * 900) | 0) + 10;
      if (nums3.indexOf(n) < 0) nums3.push(n);
    }
    const sEntry = nums3[0],
      sReal = nums3[1],
      sExit = nums3[2];
  ...
  const handlerLines = order.map(
    (o) => N.H + "[" + opnum[o] + "]=function(a) " + mutate(body[o]) + " end",
  );
  ...
  const debugAntiTamper = DEBUG_ANTITAMPER
    ? "local " +
      ...
      dbgVar +
      "=true end " + // a hook is installed -> likely stepping/tracing
      "end;"
    : "";
  ...
  const src =
    ...
    "    local " +
    N.ks +
    "=bit32.band(bit32.bxor(bit32.bxor(bit32.bxor(" +
    N.seed +
    ",(i-1)*PRIME)," +
    N.prev +
    ")," +
    ...
    "  " +
    (VM_DEBUG
      ? 'warn("[VM] entered run, program len="..tostring(#program))\n  '
      : "") +
    antiDebug +
    "\n" +
    "  " +
    ...
    "  if checksum~=nil and " +
    N.checksumOf +
    "(program)~=checksum then " +
    (VM_DEBUG
      ? 'warn("[VM] CHECKSUM MISMATCH got="..tostring(' +
        N.checksumOf +
        '(program)).." want="..tostring(checksum)) '
      : "") +
    ...
    N.K +
    "={}\n" +
    "  if CONSTS then for i=1,#CONSTS do " +
    N.K +
    "[i-1]=" +
    N.decodeNumber +
    "(CONSTS[i]) end end\n" +
  ...
  if (NESTED_VM) {
    ...
    finalSrc =
      ...
      W +
      ".run\n" +
      "local function run(program,OP,checksum,CONSTS) return " +
      G +
      "[1](program,OP,checksum,CONSTS) end\n" +
      "return { run=run, " +
      N.checksumOf +
      "=" +
      W +
...
function encodeNumber(n) {
  ...
  if (Number.isInteger(n)) {
    ...
    if (pick === 1)
      form = " " + n; // leading space (tonumber trims)
...
const BUILTIN_IDS = {
  ...
  setmetatable: { id: 26, argc: 2 },
  getmetatable: { id: 27, argc: 1 },
  rawget: { id: 28, argc: 2 },
  rawset: { id: 29, argc: 3 },
  rawequal: { id: 30, argc: 2 },
  rawlen: { id: 31, argc: 1 },
  assert: { id: 32, argc: 2 },
  select: { id: 33, argc: 2 },
  unpack: { id: 34, argc: 1 },
  "table.unpack": { id: 35, argc: 1 },
};
...
function compile(source) {
    ...
    while (i < n) {
      ...
      if (c === "-" && s[i + 1] === "-") {
        if (s[i + 2] === "[") {
          ...
          if (m) {
            const cl = "]" + m[1] + "]";
            const q = s.indexOf(cl, i + 2 + m[0].length);
            i = q < 0 ? n : q + cl.length;
            continue;
          }
        }
      ...
      if (c === "[") {
        ...
        if (m) {
          const cl = "]" + m[1] + "]";
          const q = s.indexOf(cl, i + m[0].length);
          i = q < 0 ? n : q + cl.length;
          continue;
        }
      }
  ...
  function __linner(s) {
    ...
    for (let k = 0; k < t.length; k++) {
      ...
      if (w === "for" || w === "while") st.push({ type: w, doTok: -1 });
        ...
        if (tp && (tp.type === "for" || tp.type === "while") && tp.doTok === -1)
          tp.doTok = k;
        else st.push({ type: "do" });
      } else if (w === "repeat") st.push({ type: "repeat" });
      else if (w === "until") {
        for (let z = st.length - 1; z >= 0; z--) {
          if (st[z].type === "repeat") {
            st.splice(z, 1);
            break;
          }
        ...
        const b = st.pop();
        if (b && (b.type === "for" || b.type === "while") && b.doTok >= 0)
          loops.push({ bodyStart: t[b.doTok].j, bodyEnd: t[k].i });
      }
    }
    loops.forEach(function (l) {
      l.span = l.bodyEnd - l.bodyStart;
    });
    let best = null;
    for (const ct of cs) {
      let owner = null;
      for (const l of loops) {
        if (ct.i > l.bodyStart && ct.i < l.bodyEnd) {
          if (!owner || l.span < owner.span) owner = l;
        }
      }
      if (owner) {
        if (!best || owner.span < best.span) best = owner;
      }
    }
    return best;
  }
  function __lbreak(body) {
    const t = __lt(body);
    const st = [];
    const own = [];
    for (let k = 0; k < t.length; k++) {
      ...
      if (w === "continue") {
        if (
          st.every(function (b) {
            return b.type !== "for" && b.type !== "while";
          })
        )
          own.push(t[k]);
      }
      if (w === "for" || w === "while") st.push({ type: w, dop: true });
        ...
        const tp = st[st.length - 1];
        if (tp && (tp.type === "for" || tp.type === "while") && tp.dop)
          tp.dop = false;
        else st.push({ type: "do" });
      } else if (w === "repeat") st.push({ type: "repeat" });
      else if (w === "until") {
    ...
    let o = body;
    for (const ct of own) {
      o = o.slice(0, ct.i) + "break" + o.slice(ct.j);
    }
    return o;
  }
  function __lcont(s) {
    let out = s,
      g = 0;
    while (g++ < 2000) {
      const l = __linner(out);
      if (!l) break;
      let body = out.slice(l.bodyStart, l.bodyEnd);
      body = __lbreak(body);
      out =
        out.slice(0, l.bodyStart) +
        " repeat" +
        body +
        " until true " +
        out.slice(l.bodyEnd);
  ...
  function __lprep(s) {
    ...
    o = o.replace(
      ...
      function (m, p, ws, lv, op) {
        return p + ws + lv + " = " + lv + " " + op + " ";
      },
    );
    o = __lcont(o);
    return o;
  ...
  const nodeSource = (node) =>
    node && node.range ? SRC.slice(node.range[0], node.range[1]) : null;
  const bytecode = [];
  ...
  const rollback = (snap) => {
    ...
    nextSlot = snap.nextSlot;
    varargSlot = snap.varargSlot;
  ...
  const namesUsedByNestedFns = (bodyNodes) => {
    const names = new Set();
    const walkFn = (n) => {
      // collect ALL identifiers inside a nested fn
      (function c(x) {
    ...
    (function scan(x) {
      ...
      if (x.type === "FunctionDeclaration" || x.type === "FunctionExpression") {
        walkFn(x.body);
        return;
      }
      for (const k in x) {
  ...
  const emitJunk = () => {
    ...
    if (pick === 0) {
      ...
      emitNumber(Math.floor(Math.random() * 9999));
      emit(OP.DUP);
      emit(OP.POP);
      emit(OP.POP);
    } else if (pick === 2) {
      emitNumber(Math.floor(Math.random() * 999));
      emitNumber(Math.floor(Math.random() * 999));
      emit(OP.ADD);
      emit(OP.POP);
    } else {
  ...
  const emitMBA = (v) => {
    ...
    if (!MBA_CONSTS || !Number.isInteger(v) || v < 0 || v > 100000) {
      emitNumber(v);
      return false;
    }
  ...
  function compileExpr(node) {
    if (node.type === "NumericLiteral") {
      if (MBA_CONSTS && Math.random() < 0.6) emitMBA(node.value);
      else emitNumber(node.value);
      ...
      if (scope.__upvals && scope.__upvals[node.name] !== undefined) {
        ...
        } else if (ROBLOX_PASSTHROUGH) {
          emit(OP.GETGLOBAL, encodeString(node.name));
      ...
      if (node.operator === "and") {
        ...
        throw new Error(
          "hindi sinusuportahan ang logical operator: " + node.operator,
        );
    } else if (node.type === "CallExpression") {
      compileCall(node);
      if (node.__pcallPacked) {
        const tmp = nextSlot++;
        emit(OP.STORE, tmp);
        emit(OP.LOAD, tmp);
        emitNumber(1);
  ...
  function compileFunctionExpr(node) {
    ...
    try {
      return compileFunctionExprInner(node);
    } catch (err) {
  ...
  function compileFunctionExprInner(node) {
    ...
    for (const nm of capNames) {
      scope.__upvals[nm] = ui++;
    }
    for (const u of upCaptures) {
      scope.__upvals[u.name] = ui++;
    }
  ...
  function compileArg(argNode, native) {
    if (
      native &&
      argNode &&
      (argNode.type === "FunctionDeclaration" ||
      ...
      if (src) {
        ...
        })(argNode.body || argNode);
        const bridged = [];
        for (const nm of refd) {
          // a VM local (numeric slot) or a cell-backed local in the current scope
          if (scope[nm] !== undefined && typeof scope[nm] === "number")
            bridged.push(nm);
          else if (scope.__upvals && scope.__upvals[nm] !== undefined)
            bridged.push(nm);
        }
        const gsym = "__lrp_spill_" + __spillId++;
        if (bridged.length) {
          ...
          const decls = bridged
            .map(
              (n) => "local " + n + "=__LRP_BRIDGE[" + JSON.stringify(n) + "]",
            )
            .join(";");
          const writes = bridged
            .map((n) => "__LRP_BRIDGE[" + JSON.stringify(n) + "]=" + n)
            .join(";");
          ...
          const wrapped =
            ...
            "return function(...) " +
            bridged
              .map((n) => n + "=__LRP_BRIDGE[" + JSON.stringify(n) + "]")
              .join(";") +
            "; " +
            "local __r={__f(...)}; " +
            writes +
  ...
  function compileCall(node) {
    ...
    if (
      ...
      if (src) {
        ...
        spilledFns.push({
          sym: gsym,
          src:
            "return function() local __t=table.pack(" +
            src +
            "); return __t end",
        });
    ...
    if (
      ...
      let fn = null;
      for (const qn in functions) {
        if (qn.endsWith("." + methodKey) || qn === methodKey) {
          fn = functions[qn];
          break;
        }
      }
    ...
    if (ROBLOX_PASSTHROUGH) {
      ...
      if (
        ...
        for (const a of node.arguments) compileArg(a, true);
        emit(OP.SELFCALL, [
          encodeString(node.base.identifier.name),
  ...
  function compileAssignTarget(target, valueEmitter, valueNode) {
    ...
    if (target.type === "Identifier") {
      ...
      if (scope.__upvals && scope.__upvals[target.name] !== undefined) {
        ...
        if (scope.__cells && scope.__cells.has(slot)) {
          valueEmitter();
          emit(OP.STORECELL, slot);
        } else {
          valueEmitter();
          emit(OP.STORE, slot);
        }
    ...
    if (node.type === "LocalStatement") {
      ...
      if (
        ...
        for (let i = 0; i < node.variables.length; i++) {
          ...
          if (
            tgt.type === "Identifier" &&
            bridgedEver.has(tgt.name) &&
            scope[tgt.name] !== undefined
          ) {
            if (scope.__cells && scope.__cells.has(scope[tgt.name]))
              emit(OP.LOADCELL, scope[tgt.name]);
      ...
      const __nfCap =
        scope.__nestedNames && scope.__nestedNames.has(node.variable.name);
      if (__nfCap) {
        if (!scope.__cells) scope.__cells = new Set();
        scope.__cells.add(varSlot);
        emit(OP.LOAD, varSlot);
        emit(OP.NEWCELL, varSlot);
      }
      compileBlock(node.body);
      // Read back via cell (value may have changed) before the step add.
      if (__nfCap) {
        emit(OP.LOADCELL, varSlot);
        const __t = nextSlot++;
        emit(OP.STORE, __t);
        scope.__cells.delete(varSlot);
        emit(OP.LOAD, __t);
        emit(OP.STORE, varSlot);
      }
      emit(OP.LOAD, varSlot);
      ...
      patch(jz, here());
      {
        const __ln = loopStack.pop();
        for (const b of __ln.breaks) patch(b, here());
      }
      ...
      const keyVar = node.variables[0]
        ? slotFor(node.variables[0].name)
        : nextSlot++;
      const valVar = node.variables[1]
        ? slotFor(node.variables[1].name)
        : nextSlot++;
      const top = here();
      ...
      patch(jz, here());
      {
        const __lg = loopStack.pop();
        for (const b of __lg.breaks) patch(b, here());
      }
      ...
      try {
        ...
        if (node.identifier.type === "Identifier") {
          ...
          (function collect(n) {
            ...
            if (n.type === "Identifier") __used.add(n.name);
            for (const k in n) {
              if (k === "type") continue;
              collect(n[k]);
            }
        ...
        if (__caps.length && node.identifier.type === "Identifier") {
          ...
          const closureInstr = emit(
            OP.CLOSURE,
            [0, capSlots.length].concat(capSlots),
          );
          const jmpOver = emit(OP.JMP, 0);
          const bodyAddr = here();
          bytecode[closureInstr].arg = [bodyAddr, capSlots.length].concat(
            capSlots,
          );
          const savedScope = scope,
            savedNextSlot = nextSlot,
        ...
        if (node.identifier.type === "Identifier")
          __qnameFD = node.identifier.name;
        else
          __qnameFD =
            node.identifier.base.name + "." + node.identifier.identifier.name;
...
function luraphBundle(OP, bytecode, checksum, constPool, spilledFns, usedOps) {
  ...
  if (usedOps) {
    const numToName = {};
    for (const nm of OPS) numToName[OP[nm]] = nm;
    USED_NAMES = new Set();
  ...
  if (spilledFns.length) {
    ...
    spillPrelude +=
      "local " +
      GV +
      "=(getgenv and getgenv()) or (getfenv and getfenv()) or _G;";
    ...
    for (const f of spilledFns) {
      ...
      f.src =
        "local __G=(getgenv and getgenv()) or (getfenv and getfenv()) or _G;" +
...
function wrapperBundle(src, DBG) {
  ...
  for (let i = kOrder.length - 1; i > 0; i--) {
    const j = (Math.random() * (i + 1)) | 0;
    [kOrder[i], kOrder[j]] = [kOrder[j], kOrder[i]];
  }
  const scrambled = kOrder.map((idx) => kt[idx]);
  const pos = {
    seed: kOrder.indexOf(0) + 1,
    R: kOrder.indexOf(1) + 1,
    R2: kOrder.indexOf(2) + 1,
  };
  ...
  let blobAsm = "";
  partVars.forEach((pv, i) => {
    blobAsm += "local " + pv + "={" + parts[i].join(",") + "};";
  });
  blobAsm += "local " + B + "={};";
  blobAsm +=
    "for _,_pp in ipairs({" +
  ...
  const anti =
    ...
    "if type(" +
    LS +
    ")~='function' then warn('Luripe: loadstring/load not available') return end;" +
  ...
  const loader =
    ...
    ";" +
    (DBG
      ? "warn('[Luripe] loader started - if you can read this, prints work');"
      : "") +
    J(2) +
    blobAsm +
    J(2) +
    ...
    (DBG
      ? "if " +
        ...
        "~=" +
        CK +
        " then warn('[Luripe] checksum mismatch - blob corrupted in paste') return end;"
    ...
    B +
    " do " +
    "local _b3=bit32.band(bit32.bxor(" +
    B +
    "[_i],bit32.band(" +
    RK2 +
    "+(_i-1),0xFF)),0xFF);" +
    "local _e=bit32.band(bit32.bxor(_b3," +
    RK +
    "),0xFF);" +
    "local _ks=bit32.band(bit32.bxor(bit32.bxor(" +
    SD +
    ",_i*167),_p),0xFF);" +
    ...
    "[#" +
    C +
    "+1]=string.char(bit32.band(_c,0xFF));_p=_e end;" +
    "local " +
    D +
    "=table.concat(" +
    C +
    ...
    anti +
    (DBG
      ? "warn('[Luripe] decoded '..tostring(#" + D + ")..' chars, canary OK');"
      : "") +
    "local " +
    FN +
    ",_err=" +
    ...
    "if not " +
    FN +
    " then warn('Luripe: could not compile decoded source - '..tostring(_err)) return end;" +
...
function needsWrapper(src) {
  ...
  var patterns = [/\bcoroutine\s*\./, /\bgoto\b/];
...
function pickMode() {
  ...
  if (v && (v.recommendedMode === "WRAPPER" || v.verdict === "NOT_VM_SAFE"))
    return "wrapper";
...
function downloadOutput(text) {
  ...
  try {
    ...
    /* download blocked; output is still in the box to copy */
  }
}
function run() {
  const src = document.getElementById("input").value;
  ...
  if (!src.trim()) {
    setStatus("No input to obfuscate.", "err");
    return;
  }
  const mode = pickMode();
  ...
  if (mode === "raw") {
    ...
    setStatus(
      "Raw mode - downloaded. No protection, runs as-is via executor.",
      "ok",
    );
    return;
  }
    ...
    try {
      ...
      setStatus(
        DBG
          ...
          : "Protected in Wrapper mode - downloaded. Universal compatibility (Color3/UDim2/Enum/getgc/etc all run).",
        "ok",
      );
    } catch (e) {
      outEl.value = "";
  ...
  try {
    ...
    downloadOutput(outEl.value);
    setStatus(
      "Protected in VM mode - downloaded. " +
        bytecode.length +
        " instructions, randomized opcodes" +
        (spilledFns && spilledFns.length
          ? " (" + spilledFns.length + " fn(s) hybrid-wrapped)"
          : "") +
        ".",
      "ok",
    );
    ...
    try {
      ...
      setStatus(
        "VM unsupported (" +
          e.message +
          ") - used Wrapper mode instead, downloaded.",
        "ok",
      );
    } catch (e2) {
      outEl.value = "";
...
(function () {
  ...
  if (saved) {
    const f = document.getElementById("aiKey");
    if (f) f.value = saved;
  }
  const savedProv = localStorage.getItem("luripe_ai_provider");
  if (savedProv) {
    const s = document.getElementById("aiProvider");
    if (s) s.value = savedProv;
  }
})();
...
async function aiAnalyze() {
  ...
  if (!src.trim()) {
    setStatus("No input to analyze.", "err");
    return;
  }
  if (!key) {
    setStatus("Enter your AI API key first.", "err");
    return;
  }
  ...
  supported =
    supported +
    "ALSO SUPPORTED in VM mode (added in " +
    VM_CAPABILITIES.version +
    "):\n- " +
  ...
  try {
    ...
    if (provider === "gemini") {
      ...
      if (!res.ok) {
        const t = await res.text();
        throw new Error(provider + " " + res.status + ": " + t.slice(0, 160));
      }
      const data = await res.json();
      raw =
        data.choices &&
    ...
    aiPanelShow(
      '<div style="background:var(--panel);border:1px solid var(--err);' +
...
async function aiExplainError() {
  ...
  const provider = document.getElementById("aiProvider").value;
  if (!errText) {
    setStatus("Paste the Roblox error message first.", "err");
    return;
  }
  if (!key) {
    setStatus("Enter your AI API key first.", "err");
  ...
  try {
    ...
    aiPanelShow(
      '<div style="background:var(--panel);border:1px solid var(--err);border-radius:10px;padding:14px 18px;color:var(--err);font-size:13px;">' +
        "AI error: " +
        esc(e.message) +
        "</div>",
    );
...
function renderErrorReport(rep) {
  ...
  if (rep.line != null)
    lineInfo =
      ' <span style="color:var(--text-faint);">(line ' +
      esc(rep.line) +
      ")</span>";
  aiPanelShow(
    '<div style="background:var(--panel);border:1px solid var(--border);border-radius:10px;padding:16px 20px;">' +
      ...
      esc(rep.confidence || "?") +
      "</span></div>" +
      '<div style="font-size:13px;color:var(--text);margin-bottom:10px;"><b>Cause:</b> ' +
      esc(rep.cause || "?") +
      lineInfo +
      "</div>" +
      (rep.snippet
        ? '<div style="font-size:12px;margin-bottom:10px;"><code style="color:var(--text-faint);">' +
          esc(rep.snippet) +
          "</code></div>"
        : "") +
      '<div style="font-size:13px;color:var(--text-dim);margin-bottom:8px;"><b style="color:var(--text);">Fix:</b> ' +
      esc(rep.fix || "") +
      "</div>" +
      '<div style="font-size:12px;color:var(--text-dim);">Recommended mode: <b style="color:var(--text);">' +
      esc(rep.mode || "?") +
      "</b></div>" +
      "</div>",
  );
      ...
      ? arr
          .map((x) => {
            if (x && typeof x === "object") {
              var label = esc(
                x.construct || x.name || x.type || JSON.stringify(x),
              );
              if (x.line != null)
                label +=
                  ' <span style="color:var(--text-faint);">(line ' +
...
const VM_UNSAFE_PATTERNS = [
  ...
  /\bhookfunction\b/,
  /\bhookmetamethod\b/,
  /\bgetnamecallmethod\b/,
  /\bnewcclosure\b/,
  /\bgetconnections\b/,
  /\bfiretouchinterest\b/,
  /\bfireproximityprompt\b/,
  ...
  /\bCFrame\b/,
  /\bEnum\./,
  /\btask\.(spawn|wait|delay|defer)\b/,
  /\brequire\b/,
  /\bwritefile\b/,
  /\breadfile\b/,
  /\bisfile\b/,
...
function protect(source, opts) {
  ...
  if (mode === "wrapper")
    return { mode: "wrapper", output: wrapperBundle(source, !!opts.debug) };
  if (mode === "vm") {
  ...
  if (needsWrapper(source)) {
    return {
      mode: "wrapper",
      reason: "vm-unsafe-construct",
      output: wrapperBundle(source, !!opts.debug),
    };
  }
  ...
  try {
    ...
    return {
      mode: "wrapper",
      reason: "vm-compile-failed: " + (e && e.message),
      output: wrapperBundle(source, !!opts.debug),
    };
  }
}
...
if (require.main === module) {
  ...
  let outFile =
    args[1] && !args[1].startsWith("--")
      ? args[1]
      : inFile.replace(/\.lua$/, "") + ".protected.lua";
  let mode = "auto";
  const mi = args.indexOf("--mode");
  if (mi >= 0 && args[mi + 1]) mode = args[mi + 1];
  ...
  if (out && typeof out === "object") {
    out =
      ...
      out.protected ||
      out.result ||
      (typeof out.toString === "function" ? out.toString() : "");
  }
